...
class FluxoCaixaResourceIT {
    ...
    @Mock
    private FluxoCaixaRepository fluxoCaixaRepositoryMock;

    @Mock
    private FluxoCaixaService fluxoCaixaServiceMock;
    ...
    void getAllFluxoCaixasWithEagerRelationshipsIsEnabled() throws Exception {
        when(fluxoCaixaServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restFluxoCaixaMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(fluxoCaixaServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllFluxoCaixasWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(fluxoCaixaServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restFluxoCaixaMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(fluxoCaixaRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
