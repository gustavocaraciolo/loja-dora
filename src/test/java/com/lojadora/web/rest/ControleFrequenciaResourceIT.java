...
class ControleFrequenciaResourceIT {
    ...
    @Mock
    private ControleFrequenciaRepository controleFrequenciaRepositoryMock;

    @Mock
    private ControleFrequenciaService controleFrequenciaServiceMock;
    ...
    void getAllControleFrequenciasWithEagerRelationshipsIsEnabled() throws Exception {
        when(controleFrequenciaServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restControleFrequenciaMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(controleFrequenciaServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllControleFrequenciasWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(controleFrequenciaServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restControleFrequenciaMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(controleFrequenciaRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
