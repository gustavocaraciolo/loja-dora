...
public class Entidades implements Serializable {
    ...
    @ManyToMany(fetch = FetchType.LAZY, mappedBy = "entidades")
    @Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
    ...
    public void setFluxoCaixas(Set<FluxoCaixa> fluxoCaixas) {
        if (this.fluxoCaixas != null) {
            this.fluxoCaixas.forEach(i -> i.removeEntidades(this));
        }
        if (fluxoCaixas != null) {
            fluxoCaixas.forEach(i -> i.addEntidades(this));
        }
